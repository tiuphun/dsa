...
for delivering packages and comes back to the starting point. 
There is a precedence constraint between points on the route of shipper which is represented by 
a list of m pairs (i,j): point i must be located before point j on the route (i, j = 1, 2, ..., n).
...
Line i + 2 (i = 1,...,n): contains the i-th row of the distance matrix d
